...
async function lookupCombinedGreyMarket() {
  const term = document
    .getElementById("combinedSearchInput")
    .value.trim();
  ...
  try {
    // DEV override: use local functions server if on localhost
    const url =
      window.location.hostname === "localhost"
        ? `http://localhost:9999/.netlify/functions/greyMarketLookup?term=${encodeURIComponent(
            term
          )}`
        : `/.netlify/functions/greyMarketLookup?term=${encodeURIComponent(
            term
          )}`;
